# STORY_1_PUBLIC_COPY.md

This is the public-facing copy currently rendered at
`/transformations/building-design-organization` (public mode). No internal
scaffold, no hidden evidence notes, no dev-only content — review the story
here without opening TS files.

---

**Eyebrow:** 01 — Transformation · Organizational Design
**Title:** Building the Design Organization
**Proof line:** Nikhil can build the operating system of a product design organization, not just manage design output.
...
The transformation rested on a small number of decisions made consistently over time, not on a single reorganization.

1. **Build an internal design organization rather than rely on agencies.** — Why: Durable product context compounds; outsourced work resets each project. · Tradeoff: Time, budget, and leadership ownership. · Result: A capability that grew instead of restarting.
...
> Later design-system and AI-assisted workflow maturity (Vault) contributed to an 85% reduction in QA time over two years. That outcome belongs to the AI-native Product Development story.
...
Evidence areas tracked behind this story: organizational scale (0→20, with 20 designers hired directly), field research (50+ field studies), and product-delivery quality. A broader interview count is still being confirmed before any specific number is published.
...
- This is the **public** render. The 28× Digital Gold figure and the "500+ interviews" count do **not** appear here (correctly gated).
- The internal/dev render additionally shows per-section evidence notes (e.g. the "500+ interviews needs checking" marker) — those are not part of this public copy.
